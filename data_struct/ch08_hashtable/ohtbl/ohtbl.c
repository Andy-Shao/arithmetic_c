...
int ohtbl_init(OHTbl *htbl, int positions, int (*h1)(const void *key), int (*h2)(const void *key), int (*match)(const void *key1, const void *key2), void (*destroy)(void *data)){
    ...
    //Allocate space for the hast table.
    ...
    //Set the vacated memeber to the sentinel memory address reserved for this.
...
void ohtbl_destroy(OHTbl *htbl){
    ...
    if(htbl->destroy != NULL){
        ...
        for(i = 0; i < htbl->positions; i++){
            if(htbl->table[i] != NULL && htbl->table[i] != htbl->vacated) htbl->destroy(htbl->table[i]);
        }
...
int ohtbl_insert(OHTbl *htbl, const void *data){
    ...
    //Use double hashing to has the key.
    for(i = 0; i < htbl->positions; i++){
    ...
    //Return that the has functions were selected incorrectly.
    return -1;
}
