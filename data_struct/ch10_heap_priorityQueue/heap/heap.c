...
int heap_insert(Heap *heap, const void *data){
	...
	//Heapify the tree by pushing the contenst of the new node upward.
	ipos = heap_size(heap);
	...
	while(ipos > 0 && heap->compare(heap->tree[ppos], heap->tree[ipos]) < 0){
		...
		//Move up on level in the tree to continue heapifying.
		ipos = ppos;
		ppos = heap_parent(ipos);
	}
...
int heap_extract(Heap *heap, void **data){
	...
	*data = heap->tree[0];
	...
	while(1){
		...
		if(lpos < heap_size(heap) && heap->compare(heap->tree[lpos], heap->tree[ipos]) > 0) mpos = lpos;
		else mpos = ipos;

		if(rpos < heap_size(heap) && heap->compare(heap->tree[rpos], heap->tree[mpos]) > 0) mpos = rpos;

		//When mpos is ipos, the heap property has been restored.
		if(mpos == ipos) break;
		else {
			//Swap teh contents of the current node and the selected child.
			temp = heap->tree[mpos];
