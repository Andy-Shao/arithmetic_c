...
int bitree_ins_left(BiTree *tree, BiTreeNode *node, const void *data){
    ...
    if(node == NULL){
        //Allow insertion at the root only in empty tree.
        if(bitree_size(tree) > 0) return -1;

        position = &tree->root;
    } else {
        //Normally allow insertion only at the end of a brach.
...
int bitree_ins_right(BiTree *tree, BiTreeNode *node, const void *data){
    ...
    //Determision where to insert the node.
    if(node == NULL){
    ...
    //Allocate storage for the node.
    if((new_node = (BiTreeNode *)malloc(sizeof(BiTreeNode)))) return -1;
...
int bitree_merge(BiTree *merge, BiTree *left, BiTree *right, const void *data){
    ...
    //Merge the two binary tree into a single binary tree.
