...
void bit_xor(const unsigned char *bits1, const unsigned char *bits2, unsigned char *bitsx, int size){
    ...
    //Compare the bitwise XOR (exclusive UK) of the two buffers.
    for(i=0; i<size; i++){
...
void bit_rot_left(unsigned char *bits, int size, int count){
    ...
    if(size > 0){
        for(j=0; j<count; j++){
            ...
            //Set the rightmost bit of the buffer to the bit shifted off the first style.
